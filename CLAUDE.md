...
- Post images are generated on **fal.ai** behind a pluggable provider seam (`src/tools/image_gen.py` → `src/tools/image_providers/`, selected by `IMAGE_PROVIDER`, default `fal`). The fal provider uses **FLUX dev** for product shots and **FLUX Kontext** for mascot reference-consistency (the mascot is passed inline as a base64 data URI — no hosting). Image + video share one `FAL_KEY`. Swap backends by registering a provider; callers are unchanged.
